 ...
 * It leverages {@link FirestoreRecyclerAdapter} to display students in the class and 
 * visually distinguishes between those who are currently connected to the lesson 
 * and those who are offline.
 * 
 ...
 */
public class ParticipantAdapter extends FirestoreRecyclerAdapter<User, ParticipantAdapter.ParticipantViewHolder> {
    ...
    /**
     * Initializes the adapter with the Firestore options and initial connection state.
     ...
     */
    public ParticipantAdapter(@NonNull FirestoreRecyclerOptions<User> options, List<String> connectedEmails) {
        super(options);
     ...
     */
    @Override
    protected void onBindViewHolder(@NonNull ParticipantViewHolder holder, int position, @NonNull User model) {
        holder.tvName.setText(model.getFname());

        // Binary Processing: Decode the Base64 profile picture string into a Bitmap.
        if (model.getProfilePictureBase64() != null && !model.getProfilePictureBase64().isEmpty()) {
            try {
                byte[] decodedString = Base64.decode(model.getProfilePictureBase64(), Base64.DEFAULT);
        ...
        boolean isConnected = connectedEmails.contains(model.getEmail());
        if (isConnected) {
